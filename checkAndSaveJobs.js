...
const API_STATUS_ENDPOINT = `${process.env.API_BASE_URL}/sd-queue`;
    ...
    for (const job of [...activeJobs]) {
      try {
        const response = await axios.get(
          `${API_STATUS_ENDPOINT}/${job.taskId}/status`
        );
          ...
          if (status === "completed") {
            ...
            try {
              ...
              loraUsed =
                ...
                info.extra_generation_params["Lora hashes"]
                  ? true
                  : false;
            } catch (parseError) {
              console.error(
                `Error parsing info for job ${job.taskId}:`,
                parseError
              );
            }
...
async function saveImages(taskId, images, showType, prompt, seed, loraUsed) {
  ...
  images.forEach((imgData, idx) => {
    console.log(
      `Saving image and base64 data for task ${taskId}, index ${idx}`
    );
    ...
    const isDuplicate = metadata.some(
      (entry) => entry.filename === imgFilename
    );
    if (!isDuplicate) {
